...
public class PathAcceptanceStep {
    public static ExtractableResponse<Response> 로그인이_된_거리_경로_조회_요청(TokenResponse tokenResponse, String type, long source, long target) {
    ...
    public static ExtractableResponse<Response> 로그인이_안된_거리_경로_조회_요청(String type, long source, long target) {
        return RestAssured.given().log().all().
            accept(MediaType.APPLICATION_JSON_VALUE).
            when().
            get("/paths?source={sourceId}&target={targetId}&type={type}", source, target, type).
